...
pub fn boost_keycode() -> Result<Vec<i64>, ParseIntError> {
    ...
    Ok(execute_intcode(&mut input, &[1]).unwrap())
}
pub fn sensor_boost() -> Result<Vec<i64>, ParseIntError> {
    ...
    Ok(execute_intcode(&mut input, &[2]).unwrap())
}
fn execute_intcode(v: &mut [i64], input: &[i64]) -> Option<Vec<i64>> {
...
impl Machine {
    ...
    fn execute(&mut self, input: Option<&[i64]>) -> Option<Vec<i64>> {
        use Operation::*;
...
mod test {
    ...
    fn test_full_intcode() {
        ...
        let mut machine = Machine::new(&arr);
        assert_eq!(machine.execute(None), Some(arr));

        Machine::new(&[1102, 34915192, 34915192, 7, 4, 7, 99, 0]).execute(None);

        assert_eq!(
            Machine::new(&[104, 1125899906842624, 99]).execute(None),
